// $Id$
//
//    File: JEventProcessor_pluginOfLMS.cc
// Created: Thu Nov 21 07:23:30 PM EST 2024
// Creator: zhikun (on Linux ifarm2402.jlab.org 5.14.0-427.42.1.el9_4.x86_64 x86_64)
//

#include "JEventProcessor_pluginOfLMS.h"
...
void InitPlugin(JApplication *app){
	InitJANAPlugin(app);
	app->AddProcessor(new JEventProcessor_pluginOfLMS());
}
} // "C"


//------------------
// JEventProcessor_pluginOfLMS (Constructor)
//------------------
JEventProcessor_pluginOfLMS::JEventProcessor_pluginOfLMS()
{

}

//------------------
// ~JEventProcessor_pluginOfLMS (Destructor)
//------------------
JEventProcessor_pluginOfLMS::~JEventProcessor_pluginOfLMS()
{
...
// init
//------------------
jerror_t JEventProcessor_pluginOfLMS::init(void)
{
...
// brun
//------------------
jerror_t JEventProcessor_pluginOfLMS::brun(JEventLoop *eventLoop, int32_t runnumber)
{
...
// evnt
//------------------
jerror_t JEventProcessor_pluginOfLMS::evnt(JEventLoop *loop, uint64_t eventnumber)
{
	...
	if(printAllAcceptedEventsAfterCuts){
		...
		gPad->SetGrid(1);
		my_canvas->Update();
		if(ecalHits.size() >= MinEcalSizeToAccept && nGood >= 0){
			plot->Draw("zcol");
			my_canvas->Print(TString::Format("./figures/EcalHitsEventNo_%07d_Digit.pdf", plotIndex));
			my_canvas->Print(TString::Format("./figures/EcalHitsEventNo_%07d_Digit.png", plotIndex));
			plotIndex ++;
		}
		my_canvas->Clear();
		plot->Reset();
...
// erun
//------------------
jerror_t JEventProcessor_pluginOfLMS::erun(void)
{
...
// fini
//------------------
jerror_t JEventProcessor_pluginOfLMS::fini(void)
{
