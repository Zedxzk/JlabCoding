...
namespace zhikunConstants{
    ...
    const Double_t maxTime = 16;
    const Double_t timeResolution = 4;                      // A sample width equals to 4 ns
    const Double_t timeBinsWidth = 1.0 / 10;           // bin width : 0.1 ns
    const Int_t timeBins = int(maxTime / timeBinsWidth);
    const Double_t maxEnergy = 0.1;
    const Double_t minEnergy = 0;
    const Double_t energyBinsWidth = 1.0 / 1000;         // bin width : 1 MeV
    const Int_t energyBins = int(maxEnergy / energyBinsWidth);      // bin nums , actually 1000
